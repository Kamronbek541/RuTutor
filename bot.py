...
import admin
import ktp_handlers
from ktp_plan import TAG_TO_RECOMMEND, LESSON_BY_ID, load_custom_lessons_from_db
...
def safe_delete(chat_id, message_id):
    """Delete a Telegram message if the bot is allowed to do it."""
    try:
        if message_id:
...
def reset_user_session(uid: int):
    """Clear temporary lesson/writing state so /start always starts cleanly."""
    try:
        storage.clear_lesson_session(uid)
    except Exception:
        storage.set_progress(uid, "mode", "idle")

def send_status(uid: int, chat_id, text: str):
    """Send a temporary status message and remember it for cleanup."""
    old = storage.get_progress(uid, "last_status_msg", "")
    if old:
        safe_delete(chat_id, old)
    m = bot.send_message(chat_id, text)
    storage.set_progress(uid, "last_status_msg", str(m.message_id))
    return m

def clear_status(uid: int, chat_id):
    old = storage.get_progress(uid, "last_status_msg", "")
    if old:
        safe_delete(chat_id, old)
        storage.clear_progress_key(uid, "last_status_msg")

def answer_menu(chat_id, text="Выбирай действие:"):
    bot.send_message(chat_id, text, reply_markup=kb_main())

def reset_and_show_menu(chat_id, uid: int, message_id=None, text="Выбирай действие:"):
    reset_user_session(uid)
    if message_id:
        try:
            bot.edit_message_text(text, chat_id, message_id, reply_markup=kb_main())
            return
        except Exception:
            pass
    answer_menu(chat_id, text)
...
def on_nav_menu(call):
    bot.answer_callback_query(call.id)
    reset_user_session(call.from_user.id)
...
def on_test_ans(call):
    ...
    storage.set_progress(
        uid,
        "test_results",
        add_quiz_result(storage.get_progress(uid, "test_results", "[]"), q, chosen),
    )
    storage.set_progress(uid, "test_idx", str(idx+1))
    bot.answer_callback_query(call.id, "✅ Верно!" if ok else f"❌ Правильно: {q['options'][correct_idx]}")
    _send_test_q(call.message.chat.id, call.message.message_id, uid)
...
def _finish_test(chat_id, msg_id, uid):
    ...
    review = build_answer_review(
        TEST_QUESTIONS,
        storage.get_progress(uid, "test_results", "[]"),
        show_all_correct=True,
        max_all=len(TEST_QUESTIONS),
    )
    bot.edit_message_text(
        f"✅ <b>Тест завершён!</b>\nРезультат: <b>{sc}/{len(TEST_QUESTIONS)}</b>\n"
        f"Уровень: <b>{lv}</b>{review}\n\nВыбери, как продолжить:",
        chat_id, msg_id, reply_markup=kb)
...
def on_task_ans(call):
    ...
    storage.set_progress(
        uid,
        "lesson_results",
        add_quiz_result(storage.get_progress(uid, "lesson_results", "[]"), task, chosen),
    )
    storage.set_progress(uid, "lesson_task_idx", str(tidx+1))
    bot.answer_callback_query(call.id, "✅ Верно!" if ok else f"❌ Правильно: {task['options'][correct_idx]}")
...
def _finish_tasks(chat_id, msg_id, uid, mid, lvl):
    ...
    review = build_answer_review(
        tasks,
        storage.get_progress(uid, "lesson_results", "[]"),
        show_all_correct=True,
        max_all=len(tasks),
    )
    bot.edit_message_text(
...
def on_open_start(call):
    ...
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("❌ Отмена", callback_data=f"open:cancel:{mid}:{lvl}"))
    kb.add(InlineKeyboardButton("⬅️ К уровню", callback_data=f"level:{mid}:{lvl}"))
    bot.edit_message_text(
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("open:cancel:"))
def on_open_cancel(call):
    ...
    storage.clear_progress_prefix(uid, "open_")
    storage.set_progress(uid, "mode", "idle")
    clear_status(uid, call.message.chat.id)
    bot.edit_message_text(
        "✅ Письменное задание отменено. Можно выбрать другое действие.",
        call.message.chat.id, call.message.message_id, reply_markup=kb_levels(uid, mid)
    )
...
def on_open_text(msg):
    ...
    if not text or mid not in MODULES:
        storage.clear_progress_prefix(uid, "open_")
        storage.set_progress(uid, "mode", "idle")
        bot.send_message(msg.chat.id, "Напиши текст одним сообщением или нажми /start.", reply_markup=kb_main())
        return

    u = storage.get_user(uid) or {}
    send_status(uid, msg.chat.id, "🤖 Проверяю текст...")
    try:
        result = evaluate_morphology_writing(text, u.get("language_level", "A1"), MODULES[mid]["title"], lvl)
    except Exception as e:
        storage.clear_progress_prefix(uid, "open_")
        storage.set_progress(uid, "mode", "idle")
        clear_status(uid, msg.chat.id)
        bot.send_message(msg.chat.id, f"⚠️ Ошибка ИИ: {e}", reply_markup=kb_main())
        return
...
def on_ctrl_start(call):
    ...
    storage.set_progress(uid, "ctrl_results", "[]")
...
def on_ctrl_ans(call):
    ...
    storage.set_progress(
        uid,
        "ctrl_results",
        add_quiz_result(storage.get_progress(uid, "ctrl_results", "[]"), q, chosen),
    )
...
def _finish_ctrl(uid, chat_id, msg_id):
    ...
    kb = InlineKeyboardMarkup()
    review = build_answer_review(
        MODULES[mid]["levels"][lvl]["control_test"],
        storage.get_progress(uid, "ctrl_results", "[]"),
        show_all_correct=True,
        max_all=ctrl_total,
    )
    if passed:
