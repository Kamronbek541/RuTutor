...
HINTS.update({
    "morphemics": "💡 Разбери слово по частям: корень несет общий смысл, приставка стоит перед корнем, суффикс помогает образовать новое слово, окончание меняет форму.",
    "word_formation": "💡 Сначала найди производящее слово, потом смотри, чем образовано новое: приставкой, суффиксом, сложением или сразу приставкой и суффиксом.",
    "gerund": "💡 Деепричастие отвечает на вопросы «что делая?» и «что сделав?» и обозначает добавочное действие: читая, написав, улыбаясь.",
    "reflexive": "💡 Возвратные глаголы обычно имеют -ся/-сь: учиться, готовиться, улыбаться. Проверь, возвращается ли действие к самому действующему лицу.",
})
...
def _safe_correct_idx(question: Dict[str, Any]) -> int:
    ...
    return _quiz_safe_correct_idx(question)
...
def _get_or_generate_package(lesson_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        pack = normalize_package(pack, lesson_id)
...
def register(bot):
    ...
    def kb_semesters(uid: int):
        ...
        # Show any custom semesters (4+) that have lessons
        shown_sems = {1, 2, 3}
        all_custom_sems = set()
        for l in KTP_LESSONS:
            if l.semester not in shown_sems:
                all_custom_sems.add(l.semester)
        for sem in sorted(all_custom_sems):
            lessons = semester_lessons(sem)
            if lessons:
                done = sum(1 for l in lessons if storage.get_ktp_progress(uid, l.lesson_id).get("done"))
                kb.add(InlineKeyboardButton(f"📄 Семестр {sem} ({done}/{len(lessons)})", callback_data=f"ktp:sem:{sem}"))
    ...
    def on_lesson(call):
        ...
        pack = _get_or_generate_package(lesson_id)
        lt = meta.lt or "—"
        practice_total = len(pack.get("practice") or []) or 0
        exam_total = len(pack.get("exam") or []) or 0
        status = (
            f"✅ Пройден\n"
            f"Лучшее: практика {p.get('practice_best',0)}/{practice_total} · контроль {p.get('exam_best',0)}/{exam_total} · письмо {p.get('writing_best',0)}/5"
            if int(p.get("done") or 0) == 1 else
            f"Прогресс: практика {p.get('practice_best',0)}/{practice_total} · контроль {p.get('exam_best',0)}/{exam_total} · письмо {p.get('writing_best',0)}/5"
        )
    ...
    def on_answer(call):
        ...
        storage.set_progress(
            uid,
            "ktp_results",
            add_quiz_result(storage.get_progress(uid, "ktp_results", "[]"), t, chosen),
        )
    ...
    def on_write_cancel(call):
        bot.answer_callback_query(call.id, "Письмо отменено.")
        uid = call.from_user.id
        lesson_id = call.data.split(":")[2]
        storage.clear_progress_prefix(uid, "ktp_write_")
        storage.set_progress(uid, "mode", "idle")
        clear_status(uid, call.message.chat.id)
        bot.edit_message_text("✅ Письменное задание отменено.", call.message.chat.id, call.message.message_id, reply_markup=kb_lesson_actions(lesson_id, uid), parse_mode="HTML")
    ...
    def on_write_text(msg):
        ...
        if not meta or not text:
            storage.set_progress(uid, "mode", "idle")
            storage.clear_progress_prefix(uid, "ktp_write_")
            bot.send_message(msg.chat.id, "Напиши текст одним сообщением или нажми /start.", reply_markup=kb_back_menu(), parse_mode="HTML")
            return
        ...
        try:
            ...
            bot.send_message(msg.chat.id, f"⚠️ Ошибка ИИ: {e}", reply_markup=kb_back_menu(), parse_mode="HTML")
            return
        ...
        p = storage.get_ktp_progress(uid, lesson_id)
        exam_total = len(pack.get("exam") or [])
        if int(p.get("exam_best") or 0) >= exam_pass_threshold(exam_total):
            storage.upsert_ktp_progress(uid, lesson_id, done=True)
