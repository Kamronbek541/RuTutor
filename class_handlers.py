...
def register(bot):
    ...
    def on_class_answer(call):
        ...
        storage.set_progress(
            uid,
            "cls_results",
            add_quiz_result(storage.get_progress(uid, "cls_results", "[]"), task, chosen),
        )
        storage.set_progress(uid, "cls_task_idx", str(task_idx + 1))
        bot.answer_callback_query(
            call.id,
            "✅ Верно!" if is_correct else f"❌ Правильно: {task['options'][correct_idx]}"
        )
        _send_class_task(bot, call.message.chat.id, call.message.message_id, uid, lesson)
    ...
    def _finish_class_lesson(bot, chat_id, message_id, uid, lesson):
        ...
        review = build_answer_review(
            lesson["tasks"],
            storage.get_progress(uid, "cls_results", "[]"),
    ...
    def on_class_hw_start(call):
        ...
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("❌ Отмена", callback_data="class:hw_cancel"))
        kb.add(InlineKeyboardButton("📅 Расписание", callback_data="class:schedule"))
        bot.edit_message_text(
            f"📝 <b>Домашнее задание</b>\n{lesson['emoji']} {lesson['title']}\n\n"
            f"{lesson['homework_prompt']}\n\n"
            f"<i>Напиши ответ одним сообщением. Если передумал — нажми «Отмена» или /start.</i>",
            call.message.chat.id, call.message.message_id, reply_markup=kb
        )

    @bot.callback_query_handler(func=lambda c: c.data == "class:hw_cancel")
    def on_class_hw_cancel(call):
        bot.answer_callback_query(call.id, "Домашнее задание отменено.")
        uid = call.from_user.id
        storage.clear_progress_prefix(uid, "hw_")
        storage.set_progress(uid, "mode", "idle")
        clear_status(uid, call.message.chat.id)
        bot.edit_message_text("✅ Домашнее задание отменено.", call.message.chat.id, call.message.message_id, reply_markup=kb_back_class())
    ...
    def on_homework_text(message):
        ...
        if not lesson:
            storage.set_progress(uid, "mode", "idle")
            storage.clear_progress_prefix(uid, "hw_")
            bot.send_message(message.chat.id, "Не удалось найти урок. Попробуй снова.", reply_markup=kb_back_class())
            return

        user = storage.get_user(uid) or {}
        lang_level = user.get("language_level", "A1")
        send_status(uid, message.chat.id, "🤖 Проверяю домашнее задание...")
        ...
        try:
            ...
            storage.set_progress(uid, "mode", "idle")
            storage.clear_progress_prefix(uid, "hw_")
            clear_status(uid, message.chat.id)
            bot.send_message(message.chat.id, f"⚠️ Ошибка ИИ: {e}", reply_markup=kb_back_class())
            return
